...
func validateInterfaceStateValue(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec) []metav1.StatusCause {
	...
	for idx, iface := range spec.Domain.Devices.Interfaces {
		...
		if iface.State == v1.InterfaceStateAbsent && iface.Bridge == nil && iface.SRIOV == nil && iface.VDPA == nil {
			causes = append(causes, metav1.StatusCause{
				Type:    metav1.CauseTypeFieldValueInvalid,
				Message: fmt.Sprintf("%q interface's state %q is supported only for bridge binding, sriov and vdpa", iface.Name, iface.State),
				Field:   field.Child("domain", "devices", "interfaces").Index(idx).Child("state").String(),
			})
		}
